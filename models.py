...
class ItemDB(Base):
    ...
    status = Column(
        String(20), nullable=False, default=ItemStatus.active.value, index=True
    )
    created_at = Column(
...
class ItemBase(BaseModel):
    ...
    category: str = Field(
        ..., min_length=1, max_length=100, description="Item category"
    )
    ...
    owner_id: str = Field(
        ..., min_length=1, max_length=100, description="Owner user ID"
    )
...
class ItemFeedParams(BaseModel):
    ...
    limit: int = Field(
        default=20, ge=1, le=100, description="Number of items to retrieve"
    )
    user_id: str = Field(..., min_length=1, description="User ID requesting the feed")
    exclude_item_ids: Optional[List[int]] = Field(
        default=None, description="List of item IDs to exclude"
    )
    category: Optional[str] = Field(default=None, description="Filter by category")
    distance: Optional[float] = Field(
        default=None, ge=0, description="Maximum distance in kilometers"
    )
    user_lat: Optional[float] = Field(
        default=None, ge=-90, le=90, description="User's latitude"
    )
    user_lon: Optional[float] = Field(
        default=None, ge=-180, le=180, description="User's longitude"
    )
